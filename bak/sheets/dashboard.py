...
def add_month_selector(service, spreadsheet_id):
    ...
    # 3️⃣ Default selected month (latest)
    service.spreadsheets().values().update(
        spreadsheetId=spreadsheet_id,
        range="Dashboard!O2",
        valueInputOption="USER_ENTERED",
        body={"values": [[
            '=INDEX(Z2:Z, COUNT(Z2:Z))'
        ]]}
    ).execute()

    # 4️⃣ Data validation (dropdown)
    requests = [
        {
            "setDataValidation": {
                "range": {
                    ...
                    "startRowIndex": 1,      # O2
                    "endRowIndex": 2,
                    "startColumnIndex": 14,  # Column O (0-based)
                    "endColumnIndex": 15
                },
                "rule": {
                    "condition": {
